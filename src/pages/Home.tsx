  ...
  const loadMatches = async () => {
    try {
      const {
        ...
        `).eq('is_public', true).gte('scheduled_at', new Date().toISOString()).order('scheduled_at', {
        ascending: true
      }).limit(3);
  ...
  return <div className="min-h-screen">
      ...
      <section className="px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="ghost" size="sm" className="text-primary" onClick={() => navigate('/matches')}>
            View All
          </Button>
        </div>

        {loading ? <div className="text-center py-8 text-muted-foreground">Loading matches...</div> : matches.length === 0 ? <div className="text-center py-8 text-muted-foreground">No upcoming matches</div> : <div className="space-y-3">
            {matches.map(match => <MatchCard key={match.id} id={match.id} date={formatMatchDate(match.scheduled_at)} time={formatMatchTime(match.scheduled_at)} field={match.fields?.name || 'TBD'} playersCount={getPlayerCount(match)} maxPlayers={match.max_players} fairnessScore={match.fairness_score || 0} skillLevel={match.skill_level || 'intermediate'} gameType={match.match_type || 'casual'} onViewDetails={() => navigate(`/match/${match.id}`)} />)}
          </div>}
      </section>
      ...
      <SearchingModal isOpen={isSearching} onClose={() => setIsSearching(false)} onComplete={handlePlacementComplete} selectedTime={selectedSlot} />
    </div>;
};
export default Home;